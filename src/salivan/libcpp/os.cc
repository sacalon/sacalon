...
int __sacalon__system(string cmd)
{
    return system(cmd.c_str());
}

bool __sacalon__is_x86(){
    if(sizeof(void*) == 4)
        return true;
    else
        return false;
...
bool __sacalon__is_64(){
    return !__sacalon__is_x86();
}

string __sacalon__compiler_name(){
    #ifdef __clang__
    return "clang";
    #elif __GNUC__
    return "gcc";
...
string __sacalon__arch()
{
    string comname = __sacalon__compiler_name();
