...
class Struct(object):
    ...
    def get_type_name(self):
        return "__sacalon__" + self.name + self.ptr_str
...
class Type(object):
    ...
    def __str__(self):
        return "__sacalon__%s" % (self.get_type_name())
    ...
    def get_name_for_error(self):
        ...
        type_name = self.type_name.replace("__sacalon__","")
        if self.is_ptr:
...
def return_null_according_to_type(type_, expr,name,decl=True,array_decl=False):
    ...
    if (
        ...
        if decl:
            return "%s __sacalon__%s = %s;" % (type_["expr"], name, expr["expr"])             
        return "%s = %s;\n" % (name, expr["expr"])
    return ""
